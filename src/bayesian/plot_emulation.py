...
def plot(config: emulation.EmulationConfig):
    ...
    :param EmulationConfig config: emulation configuration.
    """
    analysis_settings = config.analysis_settings

    for emulator_name, emulator_settings in config.emulation_settings.items():
        output_file = emulation.IO.output_filename(emulator_settings, analysis_settings)
        if not output_file.exists():
            logger.info(f"Emulator output does not exist: {output_file}")
            continue
        results = emulation.IO.read_emulator(emulator_settings, analysis_settings)

        plot_dir = analysis_settings.output_dir / f"plot_emulation_group_{emulator_name}"
        plot_dir.mkdir(parents=True, exist_ok=True)

        # PCA plots
        _plot_pca_reconstruction_error(results, plot_dir, emulator_settings)
        _plot_pca_reconstruction_observables(results, config, emulator_settings, plot_dir)
        _plot_pca_reconstruction_observables_per_n_pc(results, config, emulator_settings, plot_dir)
        _plot_pca_explained_variance(results, plot_dir, emulator_settings)
        _plot_pca_reconstruction_error_by_feature(results, plot_dir, config, emulator_settings)
        _plot_pca_reconstruction_error_by_feature(results, plot_dir, config, emulator_settings, fixed_y_range=True)

        # Emulator plots
        _plot_emulator_observables(results, config, emulator_settings, emulator_name, plot_dir, validation_set=False)
        _plot_emulator_observables(results, config, emulator_settings, emulator_name, plot_dir, validation_set=True)

        _plot_emulator_residuals(results, config, emulator_settings, emulator_name, plot_dir, validation_set=False)
        _plot_emulator_residuals(results, config, emulator_settings, emulator_name, plot_dir, validation_set=True)


# ---------------------------------------------------------------
def _plot_pca_explained_variance(results, plot_dir, emulator_settings):
    """
    ...
    pca = results["PCA"]["pca"]
    n_pc_max = 30
    n_pc_selected = emulator_settings.n_pc
    ...
    plt.grid(True)
    plt.plot(x, y, linewidth=2, linestyle="-", alpha=1.0, color=sns.xkcd_rgb["dark sky blue"])
    plt.plot([], [], " ", label=f"n_pc_selected= {n_pc_selected}")
    plt.legend(frameon=False)
    plt.tight_layout()
    plt.savefig(plot_dir / "PCA_explained_variance.pdf")
    plt.close()


# ---------------------------------------------------------------
def _plot_pca_reconstruction_error(results, plot_dir, emulator_settings):
    """
    Compute reconstruction error -- inverse transform and then compute residuals
    https://stackoverflow.com/questions/36567844/pca-projection-and-reconstruction-in-scikit-learn
    """
    ...
    x = range(n_pc_max)
    y = [
        np.sum((Y - scaler.inverse_transform(Y_pca[:, :n_pc].dot(pca.components_[:n_pc, :]))) ** 2, axis=1).mean()
        for n_pc in x
    ]
    ...
    plt.legend(frameon=False)
    plt.tight_layout()
    plt.savefig(plot_dir / "PCA_reconstruction_error.pdf")
    plt.close()


# ---------------------------------------------------------------
def _plot_pca_reconstruction_error_by_feature(
    results, plot_dir, emulation_config, emulator_settings, fixed_y_range: bool = False
):
    ...
    analysis_settings = emulation_config.analysis_settings
    all_observables = data_IO.read_dict_from_h5(analysis_settings.output_dir, analysis_settings.io.observables_filename)
    ...
    Y_dict = data_IO.observable_dict_from_matrix(
        Y, observables=all_observables, validation_set=False, observable_filter=observable_filter
    )
    ...
    for n_chunk in range(1, n_pc_max, n_pc_per_figure):
        ...
        ax.set_title(
            f"PCA: reconstruction error n_pc={'-'.join([str(n_pc_range[0]), str(n_pc_range[-1])])}", fontsize=14
        )
        ax.set_xlabel("PCA feature", fontsize=16)
        ax.set_ylabel("reconstruction error", fontsize=16)
        ax.grid(True)
        transform_observable_label_placement = matplotlib.transforms.blended_transform_factory(
...
# ---------------------------------------------------------------
def _plot_pca_reconstruction_observables(results, emulation_config, emulator_settings, plot_dir):
    """
    ...
    observables = data_IO.read_dict_from_h5(analysis_settings.output_dir, analysis_settings.io.observables_filename)
    Y_dict = data_IO.observable_dict_from_matrix(
        Y, observables, validation_set=False, observable_filter=observable_filter
    )
    Y_dict_truncated_reconstructed = data_IO.observable_dict_from_matrix(
        Y_reconstructed_truncated, observables, validation_set=False, observable_filter=observable_filter
    )
    ...
    colors = [
        sns.xkcd_rgb["dark sky blue"],
        sns.xkcd_rgb["denim blue"],
    ]  # sns.xkcd_rgb['light blue'], sns.xkcd_rgb['pale red'], sns.xkcd_rgb['medium green']
...
# ---------------------------------------------------------------
def _plot_pca_reconstruction_observables_per_n_pc(results, emulation_config, emulator_settings, plot_dir):
    """
...
# -------------------------------------------------------------------------------------------
def _plot_emulator_observables(
    results, emulation_config, emulator_settings, emulator_name, plot_dir, validation_set=False
):
    """
    ...
    # Get observables
    observables = data_IO.read_dict_from_h5(
        analysis_settings.output_dir, analysis_settings.io.observables_filename, verbose=False
    )

    # Get design points
    design = data_IO.design_array_from_h5(
        analysis_settings.output_dir, filename=analysis_settings.io.observables_filename, validation_set=validation_set
    )
    ...
    # Translate matrix of stacked observables to a dict of matrices per observable
    Y_dict = data_IO.observable_dict_from_matrix(
        Y, observables, validation_set=validation_set, observable_filter=observable_filter
    )

    # Get emulator predictions for this group only
    group_predictions = emulation.predict(
        design,
        emulation_config,
        analysis_settings=analysis_settings,
        emulator_results={emulator_name: results},
        merge_predictions_over_groups=False,
    )
    emulator_predictions = group_predictions[emulator_name]
    emulator_predictions_dict = data_IO.observable_dict_from_matrix(
        emulator_predictions["central_value"],
        observables,
        validation_set=validation_set,
        observable_filter=observable_filter,
    )
    ...
    if validation_set:
        ...
        # Translate matrix of stacked observables to a dict of matrices per observable
        Y_dict_truncated_reconstructed = data_IO.observable_dict_from_matrix(
            Y_reconstructed_truncated, observables, validation_set=validation_set, observable_filter=observable_filter
        )

        plot_list = [
            Y_dict["central_value"],
            Y_dict_truncated_reconstructed["central_value"],
            emulator_predictions_dict["central_value"],
        ]
...
def _plot_emulator_residuals(
    ...
    for observable_label in sorted_observable_list:
        _sqrts, _system, _observable_type, _observable, _subobserable, _centrality = data_IO.observable_label_to_keys(
            observable_label
        )

        RAA_true = np.concatenate((RAA_true, np.ravel(Y_dict["central_value"][observable_label])))
        RAA_emulator = np.concatenate(
            (RAA_emulator, np.ravel(emulator_predictions_dict["central_value"][observable_label]))
        )
        std_emulator = np.concatenate(
            (
                std_emulator,
                np.ravel(np.sqrt(np.diagonal(emulator_predictions_dict["cov"][observable_label], axis1=1, axis2=2))),
            )
        )
    ...
    ax_scatter.legend(title="", title_fontsize=16, loc="upper left", fontsize=14, markerscale=5)
    ...
    ax_scatter.plot(
        [min_val, max_val], [min_val, max_val], sns.xkcd_rgb["almost black"], alpha=0.3, linewidth=3, linestyle="--"
    )
    ...
    ax_residual.set_ylabel(
        r"$\left(R_{\rm{AA}}^{\rm{true}} - R_{\rm{AA}}^{\rm{emulator}}\right) / \sigma_{\rm{emulator}}$", fontsize=20
    )
