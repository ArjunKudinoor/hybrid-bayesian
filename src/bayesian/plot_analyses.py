...
def plot(analyses: dict[str, Any], config_file: str, output_dir: str) -> None:
    ...
    for analysis_name, analysis_config in analyses.items():
        for parameterization in analysis_config["parameterizations"]:
            ...
            configs[f"{analysis_name}_{parameterization}"] = mcmc.MCConfig.from_config_file(
...
def plot_qhat_across_analyses(
    ...
    fig, ax = plt.subplots()
    for color, (analysis_name, result), posterior, config in zip(
        colors, results.items(), posteriors.values(), configs.values()
    ):
        # TODO: Labels are hard coded...
        analysis_label = r"Jet $R_{\mathrm{AA}}$"
        if "substructure" in analysis_name:
            analysis_label = r"Jet $R_{\mathrm{AA}}$ + substructure"
        ...
        if E:
            ...
            qhat_posteriors = np.array(
                [plot_qhat.qhat_over_T_cubed(posterior_samples, config, T=T, E=E) for T in x_array]
            )
        elif T:
            xlabel = "E (GeV)"
            suffix = f"T{T}"
            ...
            qhat_posteriors = np.array(
                [plot_qhat.qhat_over_T_cubed(posterior_samples, config, T=T, E=E) for E in x_array]
            )
        ...
        if plot_mean:
            ax.plot(
                x_array,
                qhat_mean,
                color=color,  # sns.xkcd_rgb['denim blue'],
                linewidth=2.0,
                linestyle="--",
                label=f"{analysis_label}: Mean",
            )
        ...
        if plot_map:
            if E:
                qhat_map = np.array(
                    [
                        plot_qhat.qhat_over_T_cubed(mcmc.map_parameters(posterior_samples), config, T=T, E=E)
                        for T in x_array
                    ]
                )
            elif T:
            ...
            ax.plot(
                x_array,
                qhat_map,  # sns.xkcd_rgb['medium green'],
                linewidth=2.0,
                linestyle="--",
                label=f"{analysis_label}: MAP",
            )
        ...
        if plot_prior and not already_drawn_prior_credible_interval:
            ...
            ax.fill_between(
                ...
                credible_low_prior,
                credible_up_prior,
                color=color,  # color=sns.xkcd_rgb['light blue'],
                alpha=0.3,
                label=f"Prior {int(cred_level * 100)}% Credible Interval (CI)",
            )
        ...
        ax.fill_between(
            ...
            credible_low,
            credible_up,
            color=color,  # alpha=0.8 if color == "#FF8301" else 1.0, #color=sns.xkcd_rgb['light blue'],
        ...
        if target_design_point.any():
            ...
            ax.plot(x_array, qhat_truth, sns.xkcd_rgb["pale red"], linewidth=2.0, label="Target")

            qhat_closure = {}
            qhat_closure["qhat_closure_array"] = np.array(
                [
                    ((qhat_truth[i] < credible_up[i]) and (qhat_truth[i] > credible_low[i]))
    ...
    ax.legend(title=f"{label}, 0-10% central data", title_fontsize=13, loc="upper right", fontsize=12, frameon=False)
